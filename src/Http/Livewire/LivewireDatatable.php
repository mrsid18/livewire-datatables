...
class LivewireDatatable extends Component
{
    ...
    public function export()
    {
        ...
        $export->setFilename($this->export_filename . ".xlsx");
    ...
    public function getExportResultsSet()
    {
        return $this->mapCallbacks(
            ...
            return collect($this->columns())->reject(function ($value, $key) {
                ...
                $val = $item->{$value->name};
                if (isset($item->{$value->name . '_value'})) {
                    $val = $item->{$value->name . '_value'};
                }
                return [$value->label ?? $value->name => $val];
            })->all();
        });
    }
